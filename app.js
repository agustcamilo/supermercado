...
function updateLiveStock() {
    document.querySelectorAll("#catalog .card").forEach((card) => {
        ...
        if (p && badge) {
            const remaining = Math.max(0, p.stock - qtyInCart(id));
...
function addToCart(productId, qty = 1) {
    ...
    if (product.stock <= 0) {
        Swal.fire({
            icon: "info",
            title: "Sin stock",
            text: `${product.title} no tiene stock disponible.`,
        });
        return;
    }
    ...
    if (currentQty >= product.stock) {
        Swal.fire({
            icon: "info",
            title: "Sin stock",
            text: `No hay más stock disponible para ${product.title}.`,
        });
        return;
    }
    ...
    if (existing) {
        ...
        state.cart.push({ id: productId, qty: newQty });
    }
    ...
    if (toAdd < qty) {
        Toastify({
            text: `${product.title}: solo ${toAdd} por stock.`,
            duration: 2500,
        }).showToast();
    } else if (toAdd > 0) {
        Toastify({
            text: `${product.title} agregado (${toAdd}).`,
            duration: 2000,
        }).showToast();
    }
}
...
function computeTotals() {
    ...
    if (state.coupon) {
        if (state.coupon.type === "percent")
            discount = Math.round(subtotal * state.coupon.value);
...
function applyCoupon(codeRaw) {
    ...
    const coupons = {
        "BIENVENID@": { type: "percent", value: 0.1, label: "10% OFF" },
    ...
    if (!found) {
        Swal.fire({
            ...
            text: "Revisa el codigo e intenta nuevamente.",
        });
        return;
    }
    state.coupon = found;
...
function renderCartModal() {
    ...
    `;
        ...
        less.addEventListener("click", () =>
            setQty(p.id, Number(qtyInput.value) - 1)
        );
...
function handleCheckoutSubmit(e) {
    ...
    if (!validateCardNumber(order.payment.card)) {
        Swal.fire({
            ...
            text: "Verifica el numero de tarjeta.",
        });
        return;
    }
...
function setupEvents() {
    ...
    el("#applyCoupon").addEventListener("click", () =>
        applyCoupon(el("#couponInput").value)
    );
...
async function openAddProductDialog() {
    const { value: formValues } = await Swal.fire({
      ...
      <div class="form-row split">
        <div><label>Precio (ARS)</label><input id="np-price" type="number" min="0" step="1" value="0"></div>
        ...
        preConfirm: () => {
            ...
            if (!title || !description || !category) {
                Swal.showValidationMessage(
                    "Completa el titulo, descripcion y categoria"
                );
                return false;
            }
            if (!(price >= 0) || !(stock >= 0)) {
    ...
    const nextId =
        Math.max(0, ...state.products.map((p) => Number(p.id) || 0)) + 1;
    ...
    applySearchFilter();
    updateLiveStock(); // 👈 recién creado, muestra su stock correcto
    ...
    if (newProduct.stock > 0) {
        ...
        Swal.fire({
            icon: "info",
            title: "Sin stock",
            text: "No hay stock para agregar este producto.",
        });
    }
}
